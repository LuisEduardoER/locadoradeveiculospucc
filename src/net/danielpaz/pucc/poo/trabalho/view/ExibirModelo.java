      ...
      if(tdCodigo.getText().isEmpty()){
        ...
        try {
        	...
        	BD bd = new BD ("oracle.jdbc.driver.OracleDriver","jdbc:oracle:thin:@localhost:1521:xe","system", "poo2014");
                ...
                Modelo modelo = new Modelo (Integer.parseInt(tdCodigo.getText()));
                
                int auxModelo = modelo.getCodigo();
                ModeloDAO modeloDAO = new ModeloDAO (bd);
                Modelo md = modeloDAO.getModelo(auxModelo);
                tableModelo.getModel().setValueAt(md.getCodigo(), 0, 0);
                tableModelo.getModel().setValueAt(md.getDescricao(), 0, 1);
